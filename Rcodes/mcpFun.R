...
x1y <- xy[grepl(as.character(cdbb$code[bc]), 
...
x2y  <- xy[grepl(as.character(cdbb$code[bc]), 
...
sp::plot(mcp1[[ds]], border = col1, lwd = 2.25, add = TRUE)
...
FName <- paste(current_directory, "/", 
               
               
      "Results", "/", "MCP", 1,  p, ".pdf", sep = "")
...
sp::plot(mcp2[[ds]], border = col2, lwd = 2.25, add = TRUE)
