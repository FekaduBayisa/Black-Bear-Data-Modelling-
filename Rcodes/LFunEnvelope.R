...
# Simulate from L-function

envelope_simulateLfun <- local({

  
envelope_simulateLfun <- function(bear23pem0, LcrossInhom)
{
...
Envelope <- spatstat.explore::envelope(yy,  fun = LcrossInhom,  
                                       
                                       nsim = nsim,  nrank = nrank, 
                                       
                                       i = i, j = j, lambdaI = lambdaI, lambdaJ = lambdaJ,
                                       
                                       simulate = expression(rshift(yy, which = j, radius = 150)),
                                       
                                       alternative = "two.sided", global = global,   
                                       
                                       use.theory = FALSE, clipdata = TRUE,                                       
                                       
                                       savefuns = TRUE, savepatterns = TRUE, 
                                       
                                       nsim2 = nsim, verbose = TRUE)                                          
...
FName <- paste(current_directory, "/", "Results", "/", "GlobalEnvelopeNonHom2500L", 
...
Envelope <- spatstat.explore::envelope(yy,  fun = LcrossInhom,  
                              ...
                              savefuns = TRUE, savepatterns = TRUE, nsim2 = nsim, verbose = TRUE)   


current_directory <- getwd() 


FName <- paste(current_directory, "/", "Results", "/", "PointWiseEnvelopeNonHom2500L", 
